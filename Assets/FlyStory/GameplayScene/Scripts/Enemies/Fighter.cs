...
{
    private Stats.FighterStats _stats;
    ...
    public override bool Attack(out float attackCooldown)
    {
        if (IsAtGoodSpot())
        {
            GameObject rocket = MonoBehaviour.Instantiate(_stats.weapon, _transform.position + new Vector3(2, -3), Quaternion.identity, null);
            rocket.GetComponent<Missile>().SetTarget(_player);
            rocket.GetComponent<Rigidbody2D>().AddForce(new Vector2(10, 0), ForceMode2D.Impulse);
    ...
    public override void Initialize(GameObject gameObjSelf)
    {
        ...
        _stats = EnemyStats.instance.enemyTypeStats.fighter;
        _player = PlaneController.player.transform;
        _playerRB = PlaneController.player.GetComponent<Rigidbody2D>();
        _target = _player.GetComponent<PlaneController>().fighterTarget.transform;
        _target.position = PlaneController.player.transform.position + new Vector3(-(_stats.attackRange - 10), 0);
    }

    public override void Move()
    {
        direction = _transform.position - (Vector3)_playerRB.position;
        direction.Normalize();
